...
class Random(object):
    ...
    Copyright https://github.com/MostAwesomeDude/java-random
...
world_seed = 6018062811117054619
slime_seed = 987214909
radius = 16
...
def main():
    if len(sys.argv) != 3:
        print(f"usage: {sys.argv[0]} <x> <z>")
        sys.exit(1)

    x = int(sys.argv[1])
    x = int(x / 16 - 1 if x < 0 else x / 16)
    z = int(sys.argv[2])
    z = int(z / 16 - 1 if z < 0 else z / 16)

    lines = []
    for cz in range(z - radius, z + radius + 1):
        str = ""
        for cx in range(x - radius, x + radius + 1):
            is_slime = is_slime_chunk(cx, cz)
            if cx == x and cz == z:
                str += "[1;31m" if is_slime else "[31m"
            elif is_slime:
                str += "[32m"
            str += "▓▓" if is_slime else pattern(cx, cz, x, z)
            str += "[m"
        lines.append(str)
    print('\n'.join(lines))
